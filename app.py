...
# Dış API yok. Bu 11 ilçenin mahalleleri doğrudan PAS'ın içinde.
NEIGHBORHOODS = {
    "Kadıköy": [
...
def sahibinden_url(district, neighborhood="", street="", min_price="", max_price=""):
    ...
    query_parts = [x.strip() for x in (neighborhood, street) if x and x.strip()]
    if query_parts:
...
body{margin:0;padding:14px;background:#f4f5f7;color:#191919;font-family:Arial,sans-serif}
.container{max-width:850px;margin:auto}
h1{font-size:46px;margin:0;letter-spacing:-1px}
.subtitle{color:#6b7280;margin:3px 0 18px;font-size:17px}
.card{background:#fff;border-radius:16px;padding:15px;margin-bottom:14px;box-shadow:0 4px 18px rgba(0,0,0,.07)}
.title{font-weight:700;margin-bottom:10px;font-size:18px}
...
summary{padding:11px 0;font-weight:700;cursor:pointer}
...
.result a{display:block;margin-top:7px;padding:10px;border-radius:8px;background:#181818;color:white;text-align:center;text-decoration:none;font-weight:700}
.count{font-size:12px;color:#6b7280;font-weight:400}
@media(max-width:600px){.grid,.neighborhoods,.pair{grid-template-columns:1fr 1fr}}
</style>
</head>
    ...
    <div id="listMode">
        ...
        <details>
            ...
            <div class="details-body">
                ...
                <div class="small" style="margin-top:6px">Şimdilik opsiyonel metin alanıdır.</div>
            </div>
        </details>
    </div>

    <div id="mapMode" class="hidden">
        <div class="warn">Haritada merkez nokta seçebilirsiniz. Konum ve yarıçap PAS tarafından hatırlanır.</div>
        <div id="map"></div>
        <div class="pair">
...
function ensureNeighborhoodBlock(district){
    ...
    wrap.innerHTML=`
        ...
        <div class="neighborhoods">
            ${list.map(n=>`
                <label class="check">
                    <input class="neighborhoodCheck" type="checkbox"
                           data-district="${esc(district)}" value="${esc(n)}"
    ...
    wrap.querySelectorAll(".neighborhoodCheck").forEach(cb=>{
        cb.onchange=()=>{
            selectedNeighborhoods[district]=
...
function saveState(){
    const inputs={};
    ["street","min_m2","max_m2","min_price","max_price","net_m2_min","net_m2_max",
     "gross_m2_min","gross_m2_max","rooms","lat","lng","radius"].forEach(id=>{
        const el=document.getElementById(id); if(el)inputs[id]=el.value;
    });
    localStorage.setItem(STATE_KEY,JSON.stringify({
        mode:document.querySelector('input[name="mode"]:checked')?.value||"list",
        side:sideValue(),
...
function restoreState(){
    ...
    if(s.side){
        const el=document.querySelector(`input[name="side"][value="${s.side}"]`);
        if(el)el.checked=true;
    }
    ...
    if(s.inputs){
        Object.entries(s.inputs).forEach(([id,value])=>{
...
function addHidden(name,value){
    ...
    input.type="hidden"; input.name=name; input.value=value; input.className="dynamic-hidden";
...
["street","min_m2","max_m2","min_price","max_price","net_m2_min","net_m2_max",
 "gross_m2_min","gross_m2_max","rooms","radius"].forEach(id=>{
...
document.getElementById("pasForm").addEventListener("submit",event=>{
    ...
    if(mode==="list"){
        ...
        Object.entries(selectedNeighborhoods).forEach(([district,items])=>{
            items.forEach(n=>addHidden("neighborhoods",`${district}|||${n}`));
        });
    }
    saveState();
});
...
def home():
    ...
    if request.method == "POST":
        ...
        parts = []
        if min_m2 or max_m2:
            parts.append(f"Alan: {min_m2 or '-'} – {max_m2 or '-'} m²")
        if net_m2_min or net_m2_max:
            parts.append(f"Net: {net_m2_min or '-'} – {net_m2_max or '-'} TL/m²")
        if gross_m2_min or gross_m2_max:
            parts.append(f"Brüt: {gross_m2_min or '-'} – {gross_m2_max or '-'} TL/m²")
        if rooms:
