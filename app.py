...
def api_get(url):
    req = Request(url, headers={"User-Agent": "PAS/1.0", "Accept": "application/json"})
    with urlopen(req, timeout=12) as response:
        return json.loads(response.read().decode("utf-8"))

def get_istanbul_districts():
    url = f"{TURKIYE_API}/districts?provinceId=34&fields=id,name&sort=name&limit=100"
    return api_get(url).get("data", [])

def get_district_id(district_name):
    wanted = district_name.casefold().strip()
    for item in get_istanbul_districts():
        if str(item.get("name", "")).casefold().strip() == wanted:
            return item.get("id")
    return None

def get_neighborhoods(district_name):
    district_id = get_district_id(district_name)
    if not district_id:
        return []
    url = f"{TURKIYE_API}/districts/{district_id}/neighborhoods?fields=id,name&sort=name&limit=1000"
    payload = api_get(url)
    return [item.get("name") for item in payload.get("data", []) if item.get("name")]

def sahibinden_url(district, neighborhood="", street="", min_price="", max_price=""):
    base = "https://www.sahibinden.com/" + f"satilik-daire/istanbul-{slugify(district)}"
    params = {}
...
.district-head{background:#f2f3f5;padding:10px 12px;font-weight:700}
...
.result a{display:block;margin-top:7px;padding:10px;border-radius:8px;background:#181818;color:#fff;text-align:center;text-decoration:none;font-weight:700}
</style>
</head>
<body>
<div class="container">
...
{% for r in results %}
<div class="result"><strong>{{ r.title }}</strong><a href="{{ r.url }}" target="_blank" rel="noopener">Sahibinden'de aç</a></div>
{% endfor %}
{% if local_filters %}
...
<div class="small" style="margin-top:6px">Sokak/cadde seçimi şimdilik opsiyonel metin alanıdır.</div>
</div>
</details>
</div>
...
function bindDistricts(){
 document.querySelectorAll(".districtCheck").forEach(cb=>{
  cb.onchange=async()=>{
   ...
   if(cb.checked){selectedDistricts.add(cb.value);await ensureNeighborhoodBlock(cb.value)}
   else{selectedDistricts.delete(cb.value);delete selectedNeighborhoods[cb.value];document.getElementById("nb-"+slugDom(cb.value))?.remove()}
   saveState();
  };
 });
}

async function fetchNeighborhoods(district){
 const response=await fetch("/api/neighborhoods/"+encodeURIComponent(district));
 const payload=await response.json();
 if(!response.ok||!payload.ok)throw new Error(payload.error||"Mahalle listesi yüklenemedi");
 return payload.data||[];
}
async function ensureNeighborhoodBlock(district){
 const id="nb-"+slugDom(district);
 if(document.getElementById(id))return;
 const wrap=document.createElement("div");
 wrap.className="district-block";wrap.id=id;
 wrap.innerHTML=`<div class="district-head">${esc(district)} mahalleleri</div><div class="neighborhoods"><div class="small">Mahalleler yükleniyor…</div></div>`;
 document.getElementById("neighborhoodArea").appendChild(wrap);
 try{
  const neighborhoods=await fetchNeighborhoods(district);
  const selected=new Set(selectedNeighborhoods[district]||[]);
  wrap.querySelector(".neighborhoods").innerHTML=neighborhoods.map(n=>`<label class="check"><input class="neighborhoodCheck" type="checkbox" data-district="${esc(district)}" value="${esc(n)}" ${selected.has(n)?"checked":""}><span>${esc(n)}</span></label>`).join("");
  wrap.querySelectorAll(".neighborhoodCheck").forEach(cb=>{
   cb.onchange=()=>{selectedNeighborhoods[district]=[...wrap.querySelectorAll(".neighborhoodCheck:checked")].map(x=>x.value);saveState()}
  });
 }catch(err){wrap.querySelector(".neighborhoods").innerHTML=`<div class="warn">${esc(err.message)}</div>`}
}
...
async function restoreState(){
 ...
 renderDistricts();
 for(const district of selectedDistricts)await ensureNeighborhoodBlock(district);
 setupMode();
 const lat=parseFloat(document.getElementById("lat").value),lng=parseFloat(document.getElementById("lng").value);
 if(!Number.isNaN(lat)&&!Number.isNaN(lng)&&map){marker=L.marker([lat,lng]).addTo(map);map.setView([lat,lng],14)}
}
function initMap(){
...
function addHidden(name,value){
 const input=document.createElement("input");input.type="hidden";input.name=name;input.value=value;input.className="dynamic-hidden";document.getElementById("pasForm").appendChild(input);
}
...
document.getElementById("pasForm").addEventListener("submit",event=>{
 ...
 const mode=document.querySelector('input[name="mode"]:checked')?.value||"list";
 if(mode==="list"){
  if(selectedDistricts.size===0){event.preventDefault();alert("En az bir ilçe seçin.");return}
  [...selectedDistricts].forEach(d=>addHidden("districts",d));
  Object.entries(selectedNeighborhoods).forEach(([district,neighborhoods])=>neighborhoods.forEach(n=>addHidden("neighborhoods",`${district}|||${n}`)));
 }
 saveState();
});
initMap();
...
@app.route("/api/neighborhoods/<district_name>")
def neighborhoods_api(district_name):
    allowed = {d["name"] for d in DISTRICTS}
    if district_name not in allowed:
        return jsonify({"ok": False, "error": "Bu ilçe PAS listesinde bulunmuyor."}), 404
    try:
        data = get_neighborhoods(district_name)
        if not data:
            return jsonify({"ok": False, "error": "Mahalle listesi bulunamadı."}), 404
        return jsonify({"ok": True, "data": data})
    except (HTTPError, URLError, TimeoutError, ValueError):
        return jsonify({"ok": False, "error": "Mahalle servisine şu anda ulaşılamıyor."}), 502
    except Exception:
        return jsonify({"ok": False, "error": "Mahalleler yüklenirken beklenmeyen bir hata oluştu."}), 500
...
def home():
    ...
    if request.method == "POST":
        ...
        local_parts=[]
        if min_m2 or max_m2: local_parts.append(f"Alan: {min_m2 or '-'} – {max_m2 or '-'} m²")
        if net_m2_min or net_m2_max: local_parts.append(f"Net: {net_m2_min or '-'} – {net_m2_max or '-'} TL/m²")
        if gross_m2_min or gross_m2_max: local_parts.append(f"Brüt: {gross_m2_min or '-'} – {gross_m2_max or '-'} TL/m²")
        if rooms: local_parts.append(f"Oda: {rooms}")
        ...
        if mode == "list":
            ...
            for item in raw:
                ...
                if district in by_district: by_district[district].append(neighborhood)
            for district in districts:
                neighborhoods = by_district.get(district) or [""]
                for neighborhood in neighborhoods:
                    title = district + (f" · {neighborhood}" if neighborhood else "") + (f" · {street}" if street else "")
                    results.append({"title": title, "url": sahibinden_url(district, neighborhood, street, min_price, max_price)})
        else:
            lat = request.form.get("lat","").strip()
            lng = request.form.get("lng","").strip()
            radius = request.form.get("radius","3").strip()
            results.append({"title": f"Harita seçimi: {lat or '-'}, {lng or '-'} · {radius} km", "url": "https://www.sahibinden.com/satilik-daire/istanbul"})
